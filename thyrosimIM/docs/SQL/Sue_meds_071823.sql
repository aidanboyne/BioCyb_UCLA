...
md AS (
	SELECT mof.*
	  ...
	  AND OrderStartDate > '2018-01-01'
),

multiDose AS (
	-- Find all patients that have overlapping dosages
	SELECT m1.*
	FROM md m1
	JOIN md m2 ON m1.PatientEncounterCSNID = m2.PatientEncounterCSNID
	WHERE (m1.OrderStartDate <= m2.OrderEndDate AND m1.OrderEndDate >= m2.OrderStartDate)
		-- these only compares order dates within the same PatientEncounterCSNID
)

SELECT PatientID, PatientEncounterCSNID, SimpleGenericName, Strength, DiscreteFrequencyID, 
		Route, OrderStartDate, OrderEndDate, OrderDateTime 
FROM md
WHERE NOT EXISTS (SELECT 1 FROM multiDose WHERE multiDose.PatientEncounterCSNID = md.PatientEncounterCSNID)
	-- Omit patients with overlapping dosages
ORDER BY PatientEncounterCSNID
	-- 104xx rows if using PatientID
	-- 111114 rows if using PatientEncounterCSNID
	-- Number of rows: PatientEncounterCSNID > PatientID

	-- 508543 rows if not omitting multiDose patients
	-- but most of the dosages have NULL end date...